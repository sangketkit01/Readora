...
@section('content')
    ...
    <div class="container">
        ...
        <form action="{{ route('comic.delete', ['bookID' => $bookID]) }}" method="post" id="delete-form" class="form"
    ...
    @if ($data->book_status != 'block')
                    ...
                    <div class="d-flex">
                        ...
                        <div class="d-flex ms-4 mt-3 align-items-center">
                            ...
                            <strong><a class="chapter-name"
                                    href="/edit_comic_chapter/{{ $bookID }}/{{ $chapter->chapterID }}">{{ $chapter->chapter_name }}</a></strong>
                        </div>
                        <div class="d-flex ms-auto me-4 align-items-center">
                            <form
                                action="{{ route('comic.chapter_status_update', ['bookID' => $bookID, 'chapterID' => $chapter->chapterID]) }}"
                            ...
                            <button type="button"
                                ...
                                id="delete-chapter"><img src="{{ asset('novel/delete.png') }}" width="35"
                                    alt=""></button>
                            <form
                                action="{{ route('comic.delete_chapter', ['bookID' => $bookID, 'chapterID' => $chapter->chapterID]) }}"
        ...
        <div class="d-flex justify-content-center align-items-center mt-4">
            <button type="button" class="div37 text-white"
                onclick="window.location.href = `{{ route('comic.trash', ['bookID' => $bookID]) }}`">ดูถังขยะ</button>
        </div>
    @else
        <div class="d-flex justify-content-center align-items-center flex-column" style="margin-top: 50px">
            <label for="" class="text text-danger fw-bold fs-4">คอมมิคของคุณถูกบล็อกโดยผู้ดูแลระบบ</label>
            <p class="text fs-5 ">กรุณาลบคอมมิครื่องนี้หรือติดต่อผู้ดูแลระบบ</p>
        </div>
